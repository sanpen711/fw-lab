// F.w 研究所：回声 + 搭子 + 私聊模块（干净修复版）
// 重点修复：私聊发送不再使用前端旧 conversation_id，改为 target_user_id，由数据库内部创建/读取真实会话。
(function(){
  ...
  function toast(msg){
    ...
    window.__fwSocialToast = setTimeout(() => {
  ...
  function installHeaderButtons(){
    $$('.header').forEach(header => {
      ...
      if(userbar){
        header.insertBefore(actions, userbar);
      }else if(menu){
        header.insertBefore(actions, menu);
      }else{
      ...
      $('[data-fw-social-body]').innerHTML = `
        <div class="fw-social-list">
          ${data.map(n => {
            ...
            return `
              <article class="fw-social-item ${n.is_read ? '' : 'unread'}">
  ...
  async function getFriendships(){
    ...
    (data || []).forEach(f => {
      ids.push(f.requester_id, f.receiver_id);
    });
  ...
  function friendItem(f, profiles){
    ...
    if(incoming){
      actions = `
        <button class="fw-social-mini-btn dark" data-fw-accept="${f.id}">同意</button>
        <button class="fw-social-mini-btn danger" data-fw-reject="${f.id}">拒绝</button>
      `;
    }else if(accepted){
      actions = `
        <button class="fw-social-mini-btn dark" data-fw-start-chat="${esc(oid)}">私聊</button>
        <button class="fw-social-mini-btn danger" data-fw-remove-friend="${f.id}">解除</button>
      `;
    }else if(outgoing){
    ...
    return `
      <article class="fw-social-item">
        ${avatar(name, p.avatar_url, `data-fw-profile-user="${esc(oid)}"`)}
        <div class="fw-social-item-main">
          <b>${esc(name)}</b>
          <span>${esc(code)} · ${esc(statusText)}</span>
        </div>
  ...
  async function openBuddy(tab = 'friends'){
    ...
    try{
      ...
      const empty = activeTab === 'friends'
        ? '暂时还没有搭子。可以点击别人头像，加为摸鱼搭子。'
        : activeTab === 'incoming'
          ? '暂无新的搭子申请。'
          : '暂无发出的申请。';
  ...
  async function searchResearchers(keyword){
    ...
    const {data, error} = await window.fwDb.client.rpc('fw_search_profiles', {
      search_text: q
    });
      ...
      for(const p of rows){
        ...
        items.push(`
          <article class="fw-social-item">
            ...
            <div class="fw-social-item-main">
              <b>${esc(p.nickname || '低功耗研究员')}</b>
              <span>实验品编号：${esc(p.lab_code || '未设置')} · ${esc(relation)}</span>
            </div>
            <div class="fw-social-item-actions">
              ${action}
              <button class="fw-social-mini-btn" data-fw-profile-user="${esc(p.id)}">资料</button>
            </div>
          </article>
        `);
      }
  ...
  async function openProfile(userId){
    ...
    try{
      ...
      if(isSelf){
        ...
        actions = `
          <button class="fw-social-mini-btn dark" data-fw-start-chat="${esc(p.id)}">私聊</button>
          <button class="fw-social-mini-btn danger" data-fw-remove-friend="${f.id}">解除搭子关系</button>
          <button class="fw-social-mini-btn danger" data-fw-block-user="${esc(p.id)}">拉黑</button>
        `;
      }else if(pendingIn){
        actions = `
          <button class="fw-social-mini-btn dark" data-fw-accept="${f.id}">同意搭子申请</button>
          <button class="fw-social-mini-btn danger" data-fw-reject="${f.id}">拒绝</button>
        `;
      }else if(pendingOut){
        actions = `
          <button class="fw-social-mini-btn" disabled>等待对方处理</button>
          <button class="fw-social-mini-btn danger" data-fw-remove-friend="${f.id}">撤回申请</button>
        `;
      }else if(blocked){
        actions = `<button class="fw-social-mini-btn" disabled>已拉黑</button>`;
      }else{
        actions = `
          <button class="fw-social-mini-btn dark" data-fw-add-friend="${esc(p.id)}">加为搭子</button>
          <button class="fw-social-mini-btn danger" data-fw-block-user="${esc(p.id)}">拉黑</button>
        `;
      }

      $('[data-fw-social-body]').innerHTML = `
        <div class="fw-profile-card">
  ...
  async function addFriend(userId){
    try{
      const {data, error} = await window.fwDb.client.rpc('fw_send_friend_request', {
      ...
      toast(err.message || '发送申请失败。');
    }
  }

  async function acceptFriendship(id, yes){
    try{
      const {error} = await window.fwDb.client.rpc('fw_respond_friendship', {
        target_friendship_id: Number(id),
        accept_request: !!yes
      });
  ...
  async function removeFriendship(id){
    try{
      const {error} = await window.fwDb.client.rpc('fw_remove_friendship', {
        target_friendship_id: Number(id)
      });
  ...
  async function blockUser(id){
    try{
      const {error} = await window.fwDb.client.rpc('fw_block_user', {
        target_user_id: id
      });
  ...
  function openChatWindow(name){
    ...
    setTimeout(() => {
      $('[data-fw-private-form] input')?.focus();
    }, 80);
  }
  ...
  async function sendPrivateMessage(form){
    ...
    try{
      const {data, error} = await window.fwDb.client.rpc('fw_send_private_message_to_user', {
        target_user_id: currentChat.targetId,
        message_text: text
      });
      ...
      if(Number.isFinite(convId) && convId > 0){
        currentChat.conversationId = convId;
      }
  ...
  function findUserIdFromClick(target){
    ...
    if(profile && profile.dataset.fwProfileUser){
      return profile.dataset.fwProfileUser;
    }

    const roomMsg = target.closest('.fw-msg[data-user-id]');

    if(roomMsg && roomMsg.dataset.userId){
      return roomMsg.dataset.userId;
    }
    ...
    if(author){
      ...
      if(typeof getPosts === 'function'){
        ...
        if(p && (p.userId || p.authorId)){
          return p.userId || p.authorId;
        }
      }
    }
  ...
  function bindEvents(){
    document.addEventListener('click', async e => {
      ...
      if(tab){
        e.preventDefault();
        openBuddy(tab.dataset.fwBuddyTab || 'friends');
        return;
      }

      if(add){
        e.preventDefault();
        addFriend(add.dataset.fwAddFriend);
        return;
      }

      if(accept){
        e.preventDefault();
        acceptFriendship(accept.dataset.fwAccept, true);
        return;
      }

      if(reject){
        e.preventDefault();
        acceptFriendship(reject.dataset.fwReject, false);
        return;
      }

      if(remove){
        e.preventDefault();
        removeFriendship(remove.dataset.fwRemoveFriend);
        return;
      }

      if(block){
        e.preventDefault();
        blockUser(block.dataset.fwBlockUser);
        return;
      }

      if(chat){
        e.preventDefault();
        startChat(chat.dataset.fwStartChat);
        return;
      }
      ...
      if(profileEditor){
        ...
        const btn = document.querySelector('[data-fw-open]');
  ...
  async function boot(){
    ...
    window.fwDb?.client?.auth?.onAuthStateChange?.(async () => {
      await refreshMe();
      refreshBadges();
    });
  }
