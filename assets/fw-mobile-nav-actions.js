...
(function(){
  ...
  function touchesNavOrBadges(mutations){
    return mutations.some(function(m){
      return Array.from(m.addedNodes || []).some(function(node){
        if(!node || node.nodeType !== 1) return false;
        if(node.matches && node.matches('.header,.site-header,header,[data-fw-open-echo],[data-fw-open-buddy],.fw-top-badge,.fw-social-badge')) return true;
        return !!(node.querySelector && node.querySelector('.header,.site-header,header,[data-fw-open-echo],[data-fw-open-buddy],.fw-top-badge,.fw-social-badge'));
      });
    });
  }
  ...
  function boot(){
    ...
    var observer = new MutationObserver(function(mutations){
      if(!isMobile() || !touchesNavOrBadges(mutations)) return;
      clearTimeout(timer);
