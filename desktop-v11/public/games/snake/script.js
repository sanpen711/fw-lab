...
let gameLoop;
let best = Number(localStorage.getItem('fw-game-snake-best') || 0);

function updateScore() {
    document.getElementById('score').innerText = `得分：${score}　最高：${best}`;
}
...
function moveSnake() {
    ...
    if (head.x === food.x && head.y === food.y) {
        ...
        if (score > best) {
            best = score;
            localStorage.setItem('fw-game-snake-best', String(best));
        }
        updateScore();
        food = generateFood();
        if (speed > 50) speed -= 5;
