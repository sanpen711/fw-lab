...
test('本地首页保留桌面视觉和完整导航框架',async({page})=>{
  ...
  await expect(page.locator('.hero-card')).toHaveCSS('background-image',/hero-fufu-office-banner\.webp/);
  await expect(page.locator('.hero-visual')).toBeHidden();
  expect((await page.request.get('/hero-fufu-office-banner.webp')).ok()).toBeTruthy();
