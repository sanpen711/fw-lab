...
let profileCard={userId:'',profile:null,relation:null,isMember:false,reportOpen:false,busy:false};
...
function closeProfile(){const modal=$('[data-align-profile-modal]');if(modal)modal.hidden=true;profileAnchor=null;profileAnchorBox=null;profileOpenToken+=1;profileCard={userId:'',profile:null,relation:null,isMember:false,reportOpen:false,busy:false};}
...
async function openProfile(userId,anchor){
  ...
  try{const [profileResult,relation,memberResult]=await Promise.all([client.from('profiles').select('id,nickname,avatar_url,lab_code').eq('id',target).maybeSingle(),queryRelation(target),client.rpc('fw_has_active_membership',{p_user_id:target})]);if(token!==profileOpenToken||modal.hidden)return;if(profileResult.error)throw profileResult.error;if(!profileResult.data)throw new Error('这份资料暂时无法查看。');profileCard={userId:target,profile:profileResult.data,relation,isMember:memberResult.error?false:Boolean(memberResult.data),reportOpen:false,busy:false};renderProfile();}
