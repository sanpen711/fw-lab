...
let profileCard={userId:'',profile:null,relation:null,publicRows:[],publicLoaded:false,publicOpen:false,reportOpen:false,busy:false};
...
function injectStyles(){
  if($('#fw-align-style'))return;const style=document.createElement('style');style.id='fw-align-style';style.textContent=`
    ...
    .align-profile-modal[hidden],.align-avatar-lightbox[hidden]{display:none}.align-profile-card{width:min(560px,100%);max-height:min(760px,calc(100vh - 42px));padding:26px;overflow:auto}.align-profile-card>button{cursor:pointer}.align-profile-head{display:flex;align-items:center;gap:18px;padding-right:38px}.align-profile-avatar{width:96px;height:96px;min-width:96px;font-size:25px}.align-profile-avatar.zoomable{cursor:zoom-in}.align-profile-identity{min-width:0}.align-profile-identity h2{margin:0 0 7px;font-size:28px}.align-profile-code{display:inline-flex;align-items:center;min-height:28px;margin:0!important;padding:0 10px;border-radius:999px;background:#f2ede4;color:#4f4a43!important;font-size:12px}.align-profile-note{margin:16px 0 0!important;padding:12px 14px;border:1px solid rgba(28,28,24,.09);border-radius:12px;background:#f8f4ed;font-size:12px}.align-profile-status{margin:15px 0 0;color:#5f5a52;font-size:13px;font-weight:850}.align-profile-actions{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:9px;margin-top:16px}.align-profile-actions button,.align-profile-tools button,.align-profile-report button{min-height:40px;border-radius:11px;font:inherit;font-size:13px;font-weight:900;cursor:pointer}.align-profile-actions button{border:1px solid rgba(28,28,24,.14);background:#fff}.align-profile-actions button.primary{border-color:#ff8995;background:#ff8995;color:#fff}.align-profile-actions button:disabled{opacity:.5;cursor:not-allowed}.align-profile-tools{display:flex;flex-wrap:wrap;gap:7px;margin-top:13px;padding-top:13px;border-top:1px solid rgba(28,28,24,.1)}.align-profile-tools button{min-height:32px;padding:0 11px;border:0;background:transparent;color:#57524b;font-size:12px}.align-profile-tools button:hover{background:#f3eee6}.align-profile-tools button.danger{color:#a14747}.align-profile-public{margin-top:14px;padding:13px;border:1px solid rgba(28,28,24,.1);border-radius:13px;background:#fff}.align-profile-public h3{margin:0 0 9px;font-size:15px}.align-profile-public-list{display:grid;gap:7px}.align-profile-public-list button{display:grid;grid-template-columns:74px minmax(0,1fr) auto;gap:9px;align-items:center;width:100%;min-height:42px;padding:8px 10px;border:0;border-radius:9px;background:#f8f4ed;text-align:left;cursor:pointer}.align-profile-public-list small{color:#a14f4f;font-weight:900}.align-profile-public-list span{overflow:hidden;color:#37332e;font-size:12px;font-weight:780;text-overflow:ellipsis;white-space:nowrap}.align-profile-public-list time{color:#858078;font-size:10px}.align-profile-report{display:grid;gap:9px;margin-top:14px;padding:14px;border:1px solid rgba(161,71,71,.22);border-radius:13px;background:#fff7f5}.align-profile-report label{display:grid;gap:5px;color:#5e5851;font-size:12px;font-weight:900}.align-profile-report select,.align-profile-report textarea{width:100%;border:1px solid rgba(28,28,24,.16);border-radius:9px;background:#fff;padding:9px;font:inherit}.align-profile-report textarea{min-height:62px;resize:vertical}.align-profile-report button{border:0;background:#a14747;color:#fff}.align-avatar-lightbox{position:fixed;inset:0;z-index:16000;display:grid;place-items:center;padding:54px;background:rgba(10,16,14,.92);backdrop-filter:blur(8px)}.align-avatar-lightbox img{display:block;max-width:min(1100px,90vw);max-height:86vh;object-fit:contain;border-radius:16px}.align-avatar-lightbox button{position:fixed;right:24px;top:20px;width:42px;height:42px;border:1px solid rgba(255,255,255,.45);border-radius:50%;background:#14201dcc;color:#fff;font-size:28px;cursor:pointer}
    ...
    @media(max-width:900px){.align-card-grid{grid-template-columns:1fr}.align-log{grid-template-columns:1fr}.align-extra-page{padding:24px}.align-extra-head{align-items:flex-start;flex-direction:column}.align-profile-actions{grid-template-columns:1fr}.align-profile-card{padding:22px}.align-profile-public-list button{grid-template-columns:64px minmax(0,1fr)}}
...
function ensureProfileModal(){
  let modal=$('[data-align-profile-modal]');if(modal)return modal;modal=document.createElement('div');modal.className='align-profile-modal';modal.dataset.alignProfileModal='1';modal.hidden=true;modal.innerHTML='<section class="align-profile-card" role="dialog" aria-modal="true" aria-label="研究员资料"><button type="button" data-align-profile-close aria-label="关闭资料卡">×</button><div data-align-profile-body></div></section>';const lightbox=document.createElement('div');lightbox.className='align-avatar-lightbox';lightbox.dataset.alignAvatarLightbox='1';lightbox.hidden=true;lightbox.innerHTML='<button type="button" data-align-avatar-close aria-label="关闭头像">×</button><img data-align-avatar-image alt="头像原图">';document.body.append(modal,lightbox);return modal;
}
function closeProfile(){const modal=$('[data-align-profile-modal]');if(modal)modal.hidden=true;profileOpenToken+=1;profileCard={userId:'',profile:null,relation:null,publicRows:[],publicLoaded:false,publicOpen:false,reportOpen:false,busy:false};}
...
function profileStatus(relation,userId){const me=currentUser()?.id;if(!me)return'登录后可以添加搭子并发起私聊。';if(String(me)===String(userId))return'这是你自己的公开资料。';if(!relation||relation.status==='rejected')return'你们还不是搭子。';if(relation.status==='accepted')return'你们已经是搭子，可以直接私聊。';if(relation.status==='pending')return String(relation.requester_id)===String(me)?'搭子申请已发出，等待对方处理。':'对方想加你为搭子。';if(relation.status==='blocked')return String(relation.requester_id)===String(me)?'你已拉黑该研究员。':'当前无法与该研究员互动。';return'你们还不是搭子。';}
function profileActions(){
  ...
  if(!me)return`<button class="primary" type="button" data-profile-login>登录后添加搭子</button><button type="button" data-profile-public>${profileCard.publicOpen?'收起公开内容':'查看公开内容'}</button>`;
  if(String(me.id)===String(target))return`<button class="primary" type="button" data-profile-edit>编辑资料</button><button type="button" data-profile-public>${profileCard.publicOpen?'收起公开内容':'查看我的公开内容'}</button>`;
  let primary='';
  ...
  return`${primary}<button type="button" data-profile-public>${profileCard.publicOpen?'收起公开内容':'查看公开内容'}</button>`;
}
function publicContentHtml(){
  if(!profileCard.publicOpen)return'';if(!profileCard.publicLoaded)return'<section class="align-profile-public"><div class="align-empty">正在读取公开内容...</div></section>';
  const rows=profileCard.publicRows;return`<section class="align-profile-public"><h3>公开内容</h3>${rows.length?`<div class="align-profile-public-list">${rows.map(row=>`<button type="button" data-profile-open-content="${esc(row.id)}" data-profile-content-kind="${esc(row.kind)}"><small>${row.kind==='square'?'精神广场':'新闻专区'}</small><span>${esc(row.preview)}</span><time>${esc(new Date(row.created_at).toLocaleDateString('zh-CN'))}</time></button>`).join('')}</div>`:'<div class="align-empty">还没有使用账号资料发布的公开内容。</div>'}</section>`;
}
...
function renderProfile(){
  const body=$('[data-align-profile-body]');const p=profileCard.profile;if(!body||!p)return;const me=currentUser();const own=String(me?.id||'')===String(profileCard.userId);const relation=profileCard.relation;const avatar=p.avatar_url?`<div class="align-profile-avatar zoomable" data-profile-avatar-full="${esc(p.avatar_url)}" role="button" tabindex="0" aria-label="放大头像"><img src="${esc(p.avatar_url)}" alt="${esc(p.nickname||'研究员')}的头像"></div>`:`<div class="align-profile-avatar">${esc(String(p.nickname||'FW').slice(0,2).toUpperCase())}</div>`;const tools=[];if(p.lab_code)tools.push(`<button type="button" data-profile-copy-code="${esc(p.lab_code)}">复制实验品编号</button>`);if(me&&!own){tools.push(`<button type="button" data-profile-report-open>举报</button>`);if(relation?.status==='accepted')tools.push(`<button class="danger" type="button" data-profile-remove="friend">解除搭子</button>`);if(relation?.status!=='blocked')tools.push(`<button class="danger" type="button" data-profile-block>拉黑</button>`);}body.innerHTML=`<div class="align-profile-head">${avatar}<div class="align-profile-identity"><h2>${esc(p.nickname||'低功耗研究员')}</h2><p class="align-profile-code">实验品编号：${esc(p.lab_code||'未设置')}</p></div></div><p class="align-profile-status">${esc(profileStatus(relation,profileCard.userId))}</p><p class="align-profile-note">公开资料只展示昵称、头像和实验品编号，不显示邮箱等隐私信息。</p><div class="align-profile-actions">${profileActions()}</div>${tools.length?`<div class="align-profile-tools">${tools.join('')}</div>`:''}${reportFormHtml()}${publicContentHtml()}`;
}
...
async function openProfile(userId){
  const target=String(userId||'');if(!target)return;const token=++profileOpenToken;const modal=ensureProfileModal();const body=$('[data-align-profile-body]',modal);modal.hidden=false;body.innerHTML='<div class="align-empty">正在读取资料...</div>';
  try{const [profileResult,relation]=await Promise.all([client.from('profiles').select('id,nickname,avatar_url,lab_code').eq('id',target).maybeSingle(),queryRelation(target)]);if(token!==profileOpenToken||modal.hidden)return;if(profileResult.error)throw profileResult.error;if(!profileResult.data)throw new Error('这份资料暂时无法查看。');profileCard={userId:target,profile:profileResult.data,relation,publicRows:[],publicLoaded:false,publicOpen:false,reportOpen:false,busy:false};renderProfile();requestAnimationFrame(()=>$('[data-align-profile-close]',modal)?.focus());}
...
async function loadPublicContent(){
  if(!profileCard.userId||profileCard.publicLoaded)return;const target=profileCard.userId;const [square,bird]=await Promise.all([client.from('posts').select('id,content,created_at').eq('user_id',target).eq('is_deleted',false).order('created_at',{ascending:false}).limit(12),client.from('bird_posts').select('id,title,content,created_at').eq('user_id',target).eq('display_mode','profile').eq('is_deleted',false).order('created_at',{ascending:false}).limit(12)]);if(square.error)throw square.error;if(bird.error)throw bird.error;if(String(profileCard.userId)!==String(target))return;profileCard.publicRows=[...(square.data||[]).map(row=>({id:row.id,kind:'square',preview:String(row.content||'无文字内容').replace(/\s+/g,' ').slice(0,70),created_at:row.created_at})),...(bird.data||[]).map(row=>({id:row.id,kind:'bird',preview:String(row.title||row.content||'无标题内容').replace(/\s+/g,' ').slice(0,70),created_at:row.created_at}))].sort((a,b)=>new Date(b.created_at)-new Date(a.created_at)).slice(0,16);profileCard.publicLoaded=true;renderProfile();
}
...
function openAnonymousProfile(label='匿名发布者'){const modal=ensureProfileModal();profileOpenToken+=1;profileCard={userId:'',profile:null,relation:null,publicRows:[],publicLoaded:false,publicOpen:false,reportOpen:false,busy:false};modal.hidden=false;$('[data-align-profile-body]',modal).innerHTML=`<div class="align-profile-head"><div class="align-profile-avatar">匿</div><div class="align-profile-identity"><h2>${esc(label)}</h2><p class="align-profile-code">身份已隐藏</p></div></div><p class="align-profile-note">该内容使用匿名或临时笔名发布，真实账号不会公开，也不能从这里添加搭子。需要举报时，请使用内容旁边的举报入口。</p>`;}
...
function bind(){
  ...
  document.addEventListener('click',async event=>{
    const accountAvatar=event.target.closest?.('[data-account-avatar]');if(accountAvatar&&currentUser()?.id){event.preventDefault();event.stopPropagation();openProfile(currentUser().id);return;}
    const anonymousAvatar=event.target.closest?.('[data-profile-anonymous]');if(anonymousAvatar){event.preventDefault();event.stopPropagation();openAnonymousProfile(anonymousAvatar.dataset.profileAnonymous||'匿名发布者');return;}
    const userAvatar=event.target.closest?.('[data-profile-user]');if(userAvatar){event.preventDefault();event.stopPropagation();openProfile(userAvatar.dataset.profileUser);return;}
    ...
    if(event.target.closest?.('[data-align-profile-close]')||event.target.matches?.('[data-align-profile-modal]')){event.preventDefault();closeProfile();return;}
    ...
    if(event.target.closest?.('[data-profile-public]')){event.preventDefault();event.stopPropagation();profileCard.publicOpen=!profileCard.publicOpen;renderProfile();if(profileCard.publicOpen&&!profileCard.publicLoaded){try{await loadPublicContent();}catch(error){profileCard.publicLoaded=true;profileCard.publicRows=[];renderProfile();toast(error.message||'公开内容读取失败。');}}return;}
    const openContent=event.target.closest?.('[data-profile-open-content]');if(openContent){event.preventDefault();event.stopPropagation();const id=openContent.dataset.profileOpenContent;const kind=openContent.dataset.profileContentKind;closeProfile();const nav=$(`[data-nav="${kind==='bird'?'bird':'square'}"]`);nav?.click();try{if(kind==='bird'){await birdStore.load();birdStore.openPost(id);}else await feedStore.openPostById(id);}catch(error){toast(error.message||'内容已经无法查看。');}return;}
