...
let profileCard={userId:'',profile:null,relation:null,isMember:false,memberTheme:'',reportOpen:false,busy:false};
...
function injectStyles(){
  if($('#fw-align-style'))return;const style=document.createElement('style');style.id='fw-align-style';style.textContent=`
    ...
    .align-profile-card.is-vip{border-color:var(--vip-b);box-shadow:0 18px 50px rgba(0,0,0,.2),0 0 0 1px color-mix(in srgb,var(--vip-a) 30%,transparent)}.align-profile-card.is-vip .align-profile-avatar{box-shadow:0 0 0 2px #fff,0 0 0 4px var(--vip-b),0 7px 22px color-mix(in srgb,var(--vip-a) 35%,transparent)}.align-profile-card.is-vip .align-member-badge{border:1px solid color-mix(in srgb,var(--vip-b) 60%,transparent);background:linear-gradient(110deg,var(--vip-a),var(--vip-b),var(--vip-c),var(--vip-a));background-size:220% 100%;color:var(--vip-ink);letter-spacing:.08em;animation:vip-shimmer 4s linear infinite}
...
function resetProfileMemberStyle(){const card=$('.align-profile-card');card?.classList.remove('is-vip','vip-theme-rose_gold','vip-theme-black_gold','vip-theme-pink_starlight');}
function closeProfile(){const modal=$('[data-align-profile-modal]');if(modal)modal.hidden=true;resetProfileMemberStyle();profileAnchor=null;profileAnchorBox=null;profileOpenToken+=1;profileCard={userId:'',profile:null,relation:null,isMember:false,memberTheme:'',reportOpen:false,busy:false};}
...
async function openProfile(userId,anchor){
  ...
  try{const [profileResult,relation]=await Promise.all([client.from('profiles').select('id,nickname,avatar_url,lab_code').eq('id',target).maybeSingle(),queryRelation(target),membershipStore.ensureProfiles([target])]);if(token!==profileOpenToken||modal.hidden)return;if(profileResult.error)throw profileResult.error;if(!profileResult.data)throw new Error('这份资料暂时无法查看。');const memberTheme=membershipStore.themeFor(target);profileCard={userId:target,profile:profileResult.data,relation,isMember:Boolean(memberTheme),memberTheme,reportOpen:false,busy:false};renderProfile();}
...
function bind(){
  document.addEventListener('contextmenu',event=>{if(event.target.closest?.('[data-profile-user],[data-profile-anonymous]'))event.preventDefault();},true);
  ...
  document.addEventListener('click',async event=>{
    const modal=$('[data-align-profile-modal]');const profileTrigger=event.target.closest?.('[data-profile-user],[data-profile-anonymous]');const insideProfile=event.target.closest?.('.align-profile-card');const insideLightbox=event.target.closest?.('[data-align-avatar-lightbox]');if(modal&&!modal.hidden&&!insideProfile&&!insideLightbox&&!profileTrigger)closeProfile();
    ...
    if(event.target.closest?.('[data-profile-edit]')){event.preventDefault();event.stopPropagation();closeProfile();$('[data-nav="settings"]')?.click();return;}
    if(event.target.closest?.('[data-profile-login]')){event.preventDefault();event.stopPropagation();closeProfile();$('.account-button[data-open-account]')?.click();return;}
