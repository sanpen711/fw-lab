...
async function load(force=false){
  ...
  }catch(error){state.loaded=true;state.loading=false;state.error=error.message||'观鸟台读取失败。';if(showInitial)emit();if(state.posts.length)return state.posts;throw error;}
}
