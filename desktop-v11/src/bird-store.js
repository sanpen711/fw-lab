...
async function load(force=false){
  ...
  try{
    count();const posts=fail(await client.from('bird_posts').select('id,user_id,title,content,display_mode,pen_name,images,created_at,updated_at').or('is_deleted.eq.false,is_deleted.is.null').order('created_at',{ascending:false}).limit(100),'读取新闻专区失败')||[];const ids=posts.map(post=>post.id);
    let comments=[];let reactions=[];if(ids.length){count();comments=fail(await client.from('bird_comments').select('id,post_id,user_id,content,created_at').in('post_id',ids).or('is_deleted.eq.false,is_deleted.is.null').order('created_at',{ascending:true}),'读取专区评论失败')||[];count();const result=await client.from('bird_reactions').select('id,post_id,user_id,type,created_at').in('post_id',ids);reactions=result.error?[]:(result.data||[]);}
    const profileIds=Array.from(new Set(posts.filter(post=>post.display_mode==='profile').map(post=>post.user_id).concat(comments.map(comment=>comment.user_id)).filter(Boolean)));if(profileIds.length){count();const rows=fail(await client.from('profiles').select('id,nickname,avatar_url').in('id',profileIds),'读取发布者资料失败')||[];const map={...state.profiles};rows.forEach(row=>{map[String(row.id)]=row;});state.profiles=map;}
    const commentMap={};comments.forEach(row=>(commentMap[String(row.post_id)]??=[]).push(row));const reactionMap={};reactions.forEach(row=>(reactionMap[String(row.post_id)]??=[]).push(row));state.posts=posts.map(row=>({...row,images:Array.isArray(row.images)?row.images.filter(image=>image?.url):[],comments:commentMap[String(row.id)]||[],reactions:reactionMap[String(row.id)]||[]}));const changed=previousSignature!==contentSignature(state.posts,state.profiles);state.loaded=true;state.loading=false;await persistCache();if(changed||showInitial)emit();return state.posts;
  }catch(error){state.loaded=true;state.loading=false;state.error=error.message||'新闻专区读取失败。';if(showInitial)emit();if(state.posts.length)return state.posts;throw error;}
}
...
async function createPost({title,content,displayMode='profile',penName='',files=[]}){const user=requireUser();const cleanTitle=String(title||'').trim();const cleanContent=String(content||'').trim();const mode=['profile','anonymous','pen_name'].includes(displayMode)?displayMode:'profile';const alias=String(penName||'').trim();if(cleanTitle.length<2||cleanTitle.length>80)throw new Error('标题需要 2–80 个字。');if(!cleanContent||cleanContent.length>5000)throw new Error('正文需要 1–5000 个字。');if(mode==='pen_name'&&(alias.length<2||alias.length>20))throw new Error('临时笔名需要 2–20 个字。');if(files.length>20)throw new Error('最多上传 20 张图片。');return mutate(async()=>{const images=await uploadImages(files,user.id);return fail(await client.from('bird_posts').insert({user_id:user.id,title:cleanTitle,content:cleanContent,display_mode:mode,pen_name:mode==='pen_name'?alias:null,images,is_deleted:false}).select('id').single(),'发布内容失败');});}
...
async function react(postId,type){const user=requireUser();if(!['valid','seen','tissue'].includes(type))throw new Error('未知互动类型。');const post=state.posts.find(row=>String(row.id)===String(postId));if(post?.reactions.some(row=>String(row.user_id)===String(user.id)&&row.type===type))throw new Error('你已经点过这个互动了。');return mutate(()=>client.from('bird_reactions').insert({post_id:Number(postId),user_id:user.id,type}).select('id').single().then(result=>fail(result,'互动失败')));}
async function deletePost(id){return mutate(()=>client.rpc('fw_delete_own_bird_post',{p_post_id:Number(id)}).then(result=>{const data=fail(result,'删除内容失败');if(String(state.openPostId)===String(id))state.openPostId='';return data;}));}
