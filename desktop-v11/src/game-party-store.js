...
const state={loaded:false,loading:false,busy:false,error:'',parties:[],profiles:{},members:[],contacts:[],messages:[],applicationAlerts:[],openPartyId:''};
let active=false;
let channel=null;
let refreshTimer=null;

function snapshot(){return {...state,parties:[...state.parties],profiles:{...state.profiles},members:[...state.members],contacts:[...state.contacts],messages:[...state.messages],applicationAlerts:[...state.applicationAlerts]};}
...
async function load(force=false){
  ...
  try{
    count();const parties=fail(await client.from('game_parties').select('id,captain_id,game_name,platform,server_name,mode,starts_at,starts_at_text,capacity,member_count,note,requires_approval,status,created_at,updated_at').in('status',['open','full']).order('created_at',{ascending:false}).limit(100),'读取组队房间失败')||[];
    const partyIds=parties.map(row=>row.id);let members=[];let contacts=[];let messages=[];let applicationAlerts=[];
    if(currentUser()&&partyIds.length){
      ...
      count();const notices=fail(await client.from('notifications').select('id,actor_id,type,target_id,created_at').eq('user_id',currentUser().id).in('type',PARTY_ALERT_TYPES).eq('is_read',false).in('target_id',partyIds.map(String)),'读取组队提醒失败')||[];
      const pendingKeys=new Set(members.filter(row=>row.state==='pending').map(row=>`${row.party_id}:${row.user_id}`));applicationAlerts=notices.filter(row=>row.type!=='game_party_apply'||pendingKeys.has(`${row.target_id}:${row.actor_id}`));
    ...
    state.parties=parties;state.members=members;state.contacts=contacts;state.messages=messages;state.applicationAlerts=applicationAlerts;state.profiles=profiles;state.loaded=true;state.loading=false;if(state.openPartyId&&!parties.some(row=>String(row.id)===String(state.openPartyId)))state.openPartyId='';emit();return parties;
...
function activate(){active=true;if(!channel){const user=currentUser();channel=client.channel(`desktop-v11-game-parties-${user?.id||'public'}`).on('postgres_changes',{event:'*',schema:'public',table:'game_parties'},schedule).on('postgres_changes',{event:'*',schema:'public',table:'game_party_members'},schedule).on('postgres_changes',{event:'*',schema:'public',table:'game_party_messages'},schedule);if(user?.id)channel.on('postgres_changes',{event:'*',schema:'public',table:'notifications',filter:`user_id=eq.${user.id}`},schedule);channel=channel.subscribe();}return load();}
function deactivate(){active=false;clearTimeout(refreshTimer);if(channel){client.removeChannel(channel);channel=null;}}
async function markApplicationAlertsRead(partyId){const user=currentUser();if(!user?.id)return;const ids=state.applicationAlerts.filter(row=>String(row.target_id)===String(partyId)).map(row=>row.id);if(!ids.length)return;state.applicationAlerts=state.applicationAlerts.filter(row=>!ids.includes(row.id));emit();const result=await client.from('notifications').update({is_read:true}).eq('user_id',user.id).in('type',PARTY_ALERT_TYPES).in('id',ids);if(result.error){await load(true);throw new Error(`提醒状态更新失败：${result.error.message}`);}await socialStore.refreshBadges(true);}
...
async function closeParty(partyId){return mutate(async()=>fail(await client.rpc('fw_close_game_party',{p_party_id:Number(partyId)}),'删除房间失败'));}
...
authStore.subscribe(auth=>{if(!auth.ready)return;state.members=[];state.contacts=[];state.messages=[];state.applicationAlerts=[];state.loaded=false;if(channel){client.removeChannel(channel);channel=null;}emit();if(active)activate().catch(()=>{});});
