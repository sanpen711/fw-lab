...
function renderPostDetail(){
  ...
  host.innerHTML=`<div class="detail-scroll"><header class="detail-head"><b>帖子详情</b><div class="row-actions">${mine?`<button class="detail-quiet-action danger" type="button" data-delete-post="${esc(post.id)}">删除帖子</button>`:`<button class="detail-quiet-action" type="button" data-report-post="${esc(post.id)}">举报</button>`}<button class="secondary compact" type="button" data-close-post>关闭</button></div></header><article class="detail-post"><div class="detail-author-row"><div class="post-author">${avatarHtml(profile)}<b>${esc(profile.nickname||'匿名研究员')}</b></div><time>${esc(new Date(post.created_at).toLocaleString('zh-CN'))}</time></div>${richContent(post.content)}<div class="detail-post-actions"><span>${esc(post.status_tag||'今日无效')}</span><div class="reaction-row"><button class="${reaction.active.like?'active':''}" type="button" data-react="like" data-post-id="${esc(post.id)}" aria-pressed="${reaction.active.like}">点赞 ${reaction.values.like}</button></div></div></article>
    <section class="comments-section"><div class="comments-heading"><h3>评论</h3><span>${(post.comments||[]).length} 条</span></div><div class="comment-list">${tree.roots.length?tree.roots.map(root=>`<div class="comment-thread">${commentHtml(root)}${(tree.replies.get(String(root.id))||[]).map(item=>commentHtml(item,{reply:true})).join('')}</div>`).join(''):'<div class="comment-empty">还没有评论，来说两句吧。</div>'}</div>
    ${accountState.user?`<form class="comment-compose-card" data-comment-form="${esc(post.id)}">${reply?`<div class="replying">正在回复 ${esc(reply.name)}<button type="button" data-clear-reply>取消</button></div>`:''}<textarea name="content" maxlength="180" placeholder="发表评论，最多 180 字">${esc(draft.text)}</textarea>${draft.imagePreview?`<div class="image-preview small"><img src="${esc(draft.imagePreview)}" alt="待发送图片"><button class="secondary compact danger" type="button" data-comment-image-remove="${esc(post.id)}">移除</button></div>`:''}<div class="picker-block compact-picker" hidden>${sharedPicker(draft,'comment',post.id)}</div><div class="comment-compose-footer"><div class="media-tools"><label class="secondary compact file-button">图片<input type="file" accept="image/*" data-comment-image="${esc(post.id)}" hidden></label><span>也可以只发送图片或表情</span></div><button class="primary compact" type="submit" ${feedState.busy?'disabled':''}>${feedState.busy?'发送中...':'发表评论'}</button></div></form>`:'<div class="state-card small"><b>登录后参与评论</b><button class="primary compact" type="button" data-open-account>注册 / 登录</button></div>'}</section></div>`;
}
...
function birdAuthor(post){if(post.display_mode==='anonymous')return{name:'匿名发布者',avatar_url:''};if(post.display_mode==='pen_name')return{name:post.pen_name||'临时作者',avatar_url:''};const profile=birdState.profiles[String(post.user_id)]||{};return{name:profile.nickname||'发布者',avatar_url:profile.avatar_url||''};}
...
function birdCard(post){const author=birdAuthor(post);const cover=safeDirectUrl(post.images?.[0]?.url);return `<article class="bird-card" data-bird-open="${esc(post.id)}">${cover?`<div class="bird-cover"><img src="${esc(cover)}" alt="内容图片" loading="lazy">${post.images.length>1?`<span>共 ${post.images.length} 张图</span>`:''}</div>`:'<div class="bird-cover empty">暂无配图</div>'}<div><h2>${esc(post.title)}</h2><div class="post-author">${avatarHtml({nickname:author.name,avatar_url:author.avatar_url},'social-avatar mini')}<b>${esc(author.name)}</b></div><time>${esc(timeText(post.created_at))}</time></div></article>`;}
