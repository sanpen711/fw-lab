...
function renderMembershipContent(){
  ...
  host.innerHTML=`${tabs}<div class="membership-page-scroll"><div class="membership-page-grid"><main class="membership-main-stack">${status}${themes}${plans}${checkout}${membershipState.error?`<p class="membership-sync-note">${esc(membershipState.error)}</p>`:''}</main><aside class="membership-benefit-aside">${benefits}</aside></div></div>`;
}
function celebrateMembership(){
  const active=activeMembership();const user=accountState.user;if(!active||!user?.id)return;const key=`fw:desktop:v11:vip-celebrated:${user.id}:${active.expires_at}`;
  try{if(localStorage.getItem(key))return;localStorage.setItem(key,'1');}catch{return;}
  const node=document.createElement('div');node.className=`membership-celebration vip-theme-${membershipState.theme}`;node.innerHTML='<div><i>VIP</i><h2>会员身份已点亮</h2><p>专属头像光环和身份标识现已全站生效</p></div>';document.body.appendChild(node);setTimeout(()=>node.classList.add('show'),30);setTimeout(()=>{node.classList.remove('show');setTimeout(()=>node.remove(),500);},3600);
}
...
function messageBubble(row){const mine=String(row.sender_id)===String(accountState.user?.id);const sticker=decodeSticker(row.content);return `<article class="chat-message ${mine?'mine':''}"><small>${mine?'你':esc(socialState.chat.profile?.nickname||'搭子')} ${vipBadgeHtml(row.sender_id)}</small><div class="chat-bubble ${sticker?'sticker':''}">${sticker?`<img src="${esc(sticker)}" alt="表情">`:emojiText(row.content)}</div><time>${esc(timeText(row.created_at))}</time></article>`;}
function renderChat(){
